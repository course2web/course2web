...
output="$("${go_script}" --config "${config_file}" --process-not-before 2026-10-02)"
...
"${go_script}" --process-not-before 2028-02-29 >/dev/null || fail "valid leap day was rejected"

if "${go_script}" --mode process >/dev/null 2>&1; then
  fail "Phase 1 unexpectedly allowed processing"
fi
